...
/**
* Checks if the current platform version is greater than or equal to
* the desired minimum version given
*
* @param minimumVersion (string) E.g. 10.0.0.
* See [the Darwin operating system Wikipedia entry]
* (http://en.wikipedia.org/wiki/Darwin_%28operating_system%29#Release_history)
* for Mac - Darwin versions.
  ...
  var actualVersionPieces = platform.version.split('.'),
    ...
    for(var i = 0; i < numberOfPieces; i++) {
      var piece = parseInt(pieces[i], 10),
        acualPiece =parseInt(actualVersionPieces[i], 10);

      if (typeof actualPiece === 'undefined') {
        break; // e.g. 13.1 passed and acual is 13.1.0
      }
      else if (actualPiece > piece) {
        break; // doesn't matter what the next bits are, the major
        // version (or whatever) is larger
      }
      else if (actualPiece === piece) {
...
platform.is64Bit = os.arch() === 'x64' || process.env.hasOwnProperty('PROCESSOR_ARQUITEW6432');
